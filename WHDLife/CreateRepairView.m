...
@implementation CreateRepairView
...
- (void)viewDidLoad {
    ...
    [rBtn addTarget:self action:@selector(myRepairAction:) forControlEvents:UIControlEventTouchUpInside];
...
{
    MyRepairsTableView *myRepairsView = [[MyRepairsTableView alloc] init];
    myRepairsView.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:myRepairsView animated:YES];
}
...
//        [repairImageArray insertObject:smallImage atIndex:[repairImageArray count]];
...
- (IBAction)createAction:(id)sender {
    ...
    [param setValue:titleStr forKey:@"repairTitle"];
    ...
    NSString *createRepairSign = [Tool serializeSign:[NSString stringWithFormat:@"%@%@", api_base_url, api_addRepairWork] params:param];
    NSString *createRepairUrl = [NSString stringWithFormat:@"%@%@", api_base_url, api_addRepairWork];
    ...
    [request setPostValue:titleStr forKey:@"repairTitle"];
...
- (void)requestCreate:(ASIHTTPRequest *)request
{
    ...
    {
        [Tool showCustomHUD:@"报修创建完成" andView:self.parentView  andImage:@"37x-Failure.png" andAfterDelay:2];
